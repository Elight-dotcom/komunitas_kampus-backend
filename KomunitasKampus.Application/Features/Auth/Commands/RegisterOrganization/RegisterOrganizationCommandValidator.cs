...
public class RegisterOrganizationCommandValidator : AbstractValidator<RegisterOrganizationCommand>
{
    ...
    public RegisterOrganizationCommandValidator(IAccountRepository accountRepository)
    {
        _accountRepository = accountRepository;

        RuleFor(command => command.OrganizationName)
            .NotEmpty().WithMessage("Nama organisasi wajib diisi.")
            .MaximumLength(50).WithMessage("Nama organisasi maksimal 50 karakter.")
            .MustAsync(BeUniqueOrganizationName).WithMessage("Nama organisasi sudah terdaftar.");
        ...
        RuleFor(command => command.Email)
            ...
            .Must(UseAcademicDomain).WithMessage("Email organisasi harus menggunakan domain akademik .ac.id.")
    ...
    private async Task<bool> BeUniqueOrganizationName(
        string organizationName,
        CancellationToken cancellationToken
    )
    {
        return !await _accountRepository.IsOrganizationNameTakenAsync(
